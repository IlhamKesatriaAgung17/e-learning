       ...
       <div class="container-fluid">
        <div class="card">
           <div class="card-header">
            <h3 class="card-title">MATERI SEMESTER 2</h3>
            <a href="{{route('dosen.data-materi')}}" class="btn btn-primary btn-sm float-right">Kembali<i class="fas fa-plus-square"></i></a>
           </div>
           <a href="{{route('materi2.create')}}" class="btn btn-success btn-sm float-right">Tambah Data <i class="fas fa-plus-square"></i></a>
           <div class="card-body table-responsive p-0">
            ...
            @else
                <table class="table table-bordered">
                    ...
                    @foreach ($materi2 as $no => $items )
                    <tr class="bg-light">
                        <td>{{$materi2->firstitem()+$no}}</td>
                        <td>{{$items->kategori_materi}}</td>
                        <td>{{$items->nama_materi}}</td>
                        <td>{{$items->matkul}}</td>
                        <td>{{$items->semester}}</td>
                        <td><a href="{{ url('/img/materi2/semester2/'.$items->file)}}"><button class="btn btn-primary" type="button">Download File</button></a></td>
                        <td class="text-center">
                            <div class="btn-group">
                            <a href="{{route('materi2.edit',['materi2' => $items->id]) }}"><i class="fas fa-edit btn btn-warning btn-sm"></i></a>
                            &nbsp;
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-footer">
                                    <form method="POST" action="{{ route('materi2.destroy', $items->id) }}">
                                        @method('DELETE')
                                        @csrf
