           ...
           <div class="card-body table-responsive p-0">
            ...
            @else
                <table class="table table-bordered">
                    ...
                    @foreach ($materi3 as $no => $items )
                    <tr class="bg-light">
                        <td>{{$materi3->firstitem()+$no}}</td>
                        <td>{{$items->kategori_materi}}</td>
                        <td>{{$items->nama_materi}}</td>
                        <td>{{$items->matkul}}</td>
                        <td>{{$items->semester}}</td>
                        <td><a href="{{ url('/img/materi/semester3/'.$items->file)}}"><button class="btn btn-primary" type="button">Download File</button></a></td>
                        <td class="text-center">
                            <div class="btn-group">
                            <a href="{{route('materi3.edit',['materi3' => $items->id]) }}"><i class="fas fa-edit btn btn-warning btn-sm"></i></a>
                            &nbsp;
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-footer">
                                    <form method="POST" action="{{ route('materi3.destroy', $items->id) }}">
                                        @method('DELETE')
                                        @csrf
